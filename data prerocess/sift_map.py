...
outside_folder = f'/data/FocalTech/nasic9395_0606_aug_v3_siftMap_debug'
status = 'val'
...
output_full_folder_root = os.path.join(outside_folder, status, 'SiftMap_all', '') # include black 
dst_new_folder_root = os.path.join(outside_folder, status, 'SiftMap', '') # without black
...
def drawGaussian(map, kernel=5, std=0.707):
...
def drawGaussianSIFTMap(src_pts, dst_pts, kernel=7, std=1.25):
    SIFT_map = drawBlankMap(src_pts, dst_pts)
    Gaus_SIFT_map = drawGaussian(SIFT_map, kernel=kernel, std=std)

    return Gaus_SIFT_map


def SIFTwithRANSAC(img1, img2, rgb_img1, rgb_img2, threshold=0.8, MIN_MATCH_COUNT=5):
...
def job(id):
    ...
    for file in imgs:
        ...
        print("_nb_x_root", _nb_x_root)
        print("_output_nb_x_root", _output_nb_x_root)
        ...
        try:
            ...
            src_pts, dst_pts = SIFTwithRANSAC(img1, img2, src_img1, dst_img2,
                                              threshold=threshold,
                                              MIN_MATCH_COUNT=MIN_MATCH_COUNT)
            
            _GaussianSIFTMap = drawGaussianSIFTMap(src_pts, dst_pts)
            ...
            try:
                ...
                src_pts, dst_pts = SIFTwithRANSAC(img1, img2, src_img1, dst_img2,
                                                  threshold=threshold,
                                                  MIN_MATCH_COUNT=MIN_MATCH_COUNT)
                rotate_GaussianSIFTMap = drawGaussianSIFTMap(src_pts, dst_pts)
